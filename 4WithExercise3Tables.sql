...
t_page_title as (SELECT event_date, event_timestamp ,user_pseudo_id, value.int_value as page_title
FROM `vidalvidal.analytics_267182121.events_20231015` , UNNEST(event_params) as event_params 
...
SELECT event_date, event_timestamp ,user_pseudo_id, sesiones, page_title, payment_type
FROM t_sesiones
INNER JOIN t_page_title
USING (event_date, event_timestamp ,user_pseudo_id)
INNER JOIN  t_payment_type
USING (event_date, event_timestamp ,user_pseudo_id)
